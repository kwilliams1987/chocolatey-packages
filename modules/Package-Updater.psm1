...
function Update-GithubPackage
{
    ...
    $localHash = (Select-String -Path "$PackageDirectory\tools\chocolateyInstall.ps1" -Pattern $CRegex | Select-Object -First 1).Matches.Groups[1].Value.ToLower();
    $localVer = (Select-String -Path "$PackageDirectory\tools\chocolateyInstall.ps1" -Pattern $IRegex | Select-Object -First 1).Matches.Groups[0].Value;
    $onlineHash = (Get-FileHash $target -Algorithm SHA256).Hash.ToLower();

    ((Get-Content -Path "$PackageDirectory\tools\chocolateyInstall.ps1" -Raw) -replace $localHash, $onlineHash).Trim() | Set-Content "$PackageDirectory\tools\chocolateyInstall.ps1";
    ((Get-Content -Path "$PackageDirectory\tools\chocolateyInstall.ps1" -Raw) -replace $localVer, $newver).Trim() | Set-Content "$PackageDirectory\tools\chocolateyInstall.ps1";
